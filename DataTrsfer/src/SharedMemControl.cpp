...
SharedMemControl::SharedMemControl()
{
    ...
    m_pReadSemaphoreFull = nullptr;
    m_pReadSemaphoreEmpty = nullptr;
    m_pWriteSemaphoreFull = nullptr;
    m_pWriteSemaphoreEmpty = nullptr;

    m_pReadSharedQueue = nullptr;
    m_pWriteSharedQueue = nullptr;

    void* ret_m = memset(&m_strtIpcConfig, 0, sizeof(IPC_CONFIG));
    if (nullptr == ret_m) ERROR("memset() fail"); 
...
SharedMemControl::~SharedMemControl()
{
    DEBUG("SharedMemControl Destroy()");
    DeleteReadShm();
    DeleteWriteShm();
}
...
// ********************************************************************
// * 함 수 명: WakeUpSemWaitBlocking
// * 설    명: 안전한 종료를 위해 
// *           종료전에 SemWait blocking 상태의 세마포어를 깨운다.
// * 작 성 자: KJH
// * 작성날짜: 2025. 07. 13
// ********************************************************************
void SharedMemControl::WakeUpSemWaitBlocking()
{
    sem_post(m_pReadSemaphoreFull);
    sem_post(m_pWriteSemaphoreEmpty);
